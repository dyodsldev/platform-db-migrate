{{
    config(
        materialized='view',
        tags=['staging', 'patients']
    )
}}

WITH source AS (
    SELECT * FROM {{ ref('medicaldb.patients') }}
),
...
cleaned AS (
    SELECT
        ...
        -- IDENTIFIERS
        -- ============================================
        _id::text AS mongodb_patient_id,
        ...
        -- ============================================
        diagnosis,
        to_timestamp(createdTime / 1000.0) AS diagnosis_date,
